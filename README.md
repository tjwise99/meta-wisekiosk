...
and the documentation guards (`just verify` and the pre-commit hook) need **PyYAML** for `python3` —
all of them refuse rather than skip when one is missing. On an externally-managed (PEP 668)
`python3`, install PyYAML with a virtualenv, the distro's `python3-yaml`, or
