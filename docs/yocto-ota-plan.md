...
| Out | Why |
| LXDE, lightdm, openbox, lxsession | `surf` is a systemd unit started against a bare `Xorg`. The Raspbian autostart chain (kiosk-reference `raspbian-card.md`) disappears entirely |
...
> by lightdm today (the Raspbian card notes, kiosk-reference `raspbian-card.md`), not in any config
...
2. **First boot, if absent:** the same unit looks for `kiosk.conf` on the FAT boot partition and
   copies it in. That partition is readable and writable from any card reader — which is
...
Cross-build on a workstation; never on the Pi. WebKit is the long pole — expect tens of GB of
...
The build host wants roughly **8+ cores, ~8GB+ RAM, and tens of GB of free disk** — WebKit is the
long pole on all three.
...
- **RAM is the binding constraint, not disk** — a full build needs roughly 100–150GB of free disk,
  but WebKit's link step wants multiple GB *per linker process*, and meta-autonomos's
  `reference.yaml` sets `PARALLEL_MAKE` to `cpu_count()`. On a build host without RAM to match its
  core count, that combination OOMs hours in. Override it (below), and raise the build host's memory
  cap if the tooling imposes one.
...
- **Copy the `.wic.bz2` off the build host and write it to the card with any imaging tool.** The
  build host itself need not have a card reader attached.
...
**Step 1 — fork and branch (5 min).** Work on a branch of the meta-autonomos repo, not in it. `just`
...
`wpa_passphrase <ssid> <psk>`. **`secrets.yaml` is gitignored upstream; confirm that holds in your
...
and it catches the aarch64 assumptions in the upstream Pi layer before they cost hours.
...
**Step 6 — flash.** Copy the `.wic.bz2` off the build host and write it to the spare card with any
imaging tool.
...
| 2 | Watch for `wlan0` and the DHCP lease | An address on `<WIFI_SSID>` | The console shows why — `wpa_supplicant` failing, firmware missing, wrong PSK. All readable |
...
|---|---|---|
| Parse check | Layer will not parse for `raspberrypi0-wifi` | An aarch64 assumption in the upstream Pi layer. Cheap to fix, do it now |
...
| Phase 0a | Build dies with the OOM killer | Host, not target. Lower `PARALLEL_MAKE`, raise the build host's memory cap, resume — sstate means you do not start over |
...
2. **Device scope — resolved (owner, 2026-08-14):** the board is in scope and permanent. It is the
   *backend* it serves (MagicMirror) that is being superseded by WiseKiosk; the device stays, and
   Yocto + OTA is its delivery path going forward.
