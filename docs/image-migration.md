...
| `kiosk-journal-flush.service`, `99-kiosk-journal.conf` | journal flush + symlink | `kiosk-journal` recipe (still **uncommitted** in the build tree) |
| `kiosk-bootprofile.service`, the boot sampler | this session's diagnostic | **`kiosk-bootprof` recipe** — installed, *not* enabled. An earlier draft of this row said "do not migrate"; that was wrong, the boot work is ongoing and the tool is how it is done |
...
## Traced this session, not yet acted on — the systemd → WebKit chain, and a feature-flag inversion
...
|---|---|---|
| **`panic=10` on the kernel cmdline** | `CMDLINE:append = " panic=10"` — **already added** to `kiosk-zero-w.yaml`, uncommitted | turns an unrecoverable early panic into a **60 s outage, proven** |
...
## The build tree is uncommitted

`~/meta-wisekiosk`, branch `zero-w-port`:

```
 M README.md
 M kiosk-zero-w.yaml
?? meta-autonomos-core/recipes-core/kiosk-journal/
?? meta-autonomos-core/recipes-core/kiosk-soak/
?? meta-autonomos-raspberrypi/recipes-core/rauc/files/raspberrypi0-wifi/
```

Three recipes that the running image depends on are **untracked**. The image was built from them, so
the device and the tree agree today and nothing in `git` records why. A clean checkout of this branch
does not reproduce the card that is currently in the device.

**Commit these before building anything else.** Until then, the reproducibility claim behind the
whole A/B scheme is not true.
