...
> the original observation was presumably true when written, and nothing here identified
...
| **busybox syslog/klogd** | **~0.23 s boot** (share of the same −1.92 s), and stops duplicate logging onto the SD card | `syslog` active 230 ms; `busybox-klogd` also active (93 ms measured); journald already carries these records | `VIRTUAL-RUNTIME_base-utils-syslog = ""` |
...
`MACHINE_FEATURES:remove = "… touchscreen"`. Not masked; small; not costed.
...
Masking it and replacing the symlink with a static `nameserver <GATEWAY_IP>`, n=3 against the arm
below it:
