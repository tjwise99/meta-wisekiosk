...
> **Recipe 1 (Chromium DevTools / CDP) has moved to `raspbian-card.md`.** The Yocto device runs
> `surf`/WebKit, which has no remote-debugging endpoint and no CDP equivalent — reach for Recipe 9
> (screenshot) instead to read what the kiosk is actually showing.
...
```bash
for i in $(seq 1 254); do (ping -c1 -W1 <LAN_PREFIX>.$i >/dev/null 2>&1 &); done; sleep 5
...
> **Recipe 4 (recovering SSH access to the Raspbian card) and Recipe 5 (launching Chromium as a
> GUI process over SSH) have moved to `raspbian-card.md`.** Neither applies to the Yocto device: it ships `sshd` enabled by default, and
> its browser is launched by a systemd unit, not an ad hoc SSH session.
...
`/home/pi/kiosk-soak.log`, driven by `kiosk-soak.timer`, running as `pi`. It exists because soak
questions were being answered from two hand-taken samples hours apart, which cannot separate *flat*
from *leaking slowly*.

> This paragraph and the two commands below describe the **Raspbian card's** sampler and its
> `/home/pi` path — see `raspbian-card.md` for that card. The Yocto device runs a ported sampler
> logging to `/data/kiosk-soak.log` instead (`/data` survives an A/B update; `/home/pi` does not
> exist on this image). The interpretation guidance in the rest of this recipe — the fields, the
> summary fields, and the "things that bite" below — applies to both; only the log path and the
> `--summary` output (the ported version has no `slope excl hour0` field yet) differ.
...
tools/kiosk-ssh.sh 'tail -3 /data/kiosk-soak.log'
```
...
- **Linux truncates process names to 15 characters** — on the Raspbian card, `pgrep -x
  chromium-browser` finds nothing because the name is `chromium-browse`. Use `-f` with a path
  pattern whenever a binary name is long enough to be truncated.
...
- **On the Raspbian card, `mmClient.sh` sleeps 10s between browser restarts**, so a single `pgrep`
  landing in that gap reads as "the kiosk is down" — see `raspbian-card.md`. Sample repeatedly rather
  than trusting one read, on either card.
...
A frontend change can be seconds rather than a reboot:
...
3. Reload and check for errors. On the Raspbian card, read the console via `cdp.py` (see
   `raspbian-card.md`); the Yocto device's `surf`/WebKit has no CDP equivalent, so confirm via
   Recipe 9's screenshot-and-content check instead.
