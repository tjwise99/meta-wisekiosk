// Ablation B: marquee animation and compositor-layer hint removed.
//
// This arm is UNMEASURED and its number must not be read as a result. It injects a
// <style> element, and the mirror serves style-src 'self' -- only a style *attribute*
// survives that policy -- and it carries no in-band landed-check, so a dropped rule and
// a real null are indistinguishable here. Kept as the record of what was run.
(function () {
  ...
  // ABLATION B: remove the marquee animation and its permanent compositor layer.
  // The {#key} remount, the scrollWidth/clientWidth reads and the custom-property
  // write all still happen exactly as shipped.
  (function () {
    function inject() {
      if (!document.head) { origSetTimeout(inject, 100); return; }
      var st = document.createElement('style');
      st.textContent = '.ride-name-text{animation:none !important;will-change:auto !important;}';
