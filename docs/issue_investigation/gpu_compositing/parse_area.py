...
PAT = (r'FA\|slot(-?\d+)\|mech(\w+)'
       r'\|S0_' + S + r'\|S1_' + S + r'\|S2_' + S + r'\|S3_' + S + r'\|S4_' + S)
COND = [0, 1, 2, 1, 0]
NAME = {0: 'full app', 1: 'fullscreen anim', 2: 'fullscreen static'}
DUR = 80.0

txt = open(sys.argv[1]).read().replace('"', '')
lines = [l for l in txt.splitlines() if 'FA|' in l]
if not lines:
    sys.exit("no FA| payload in " + sys.argv[1])
m = re.search(PAT, lines[-1])
if not m:
    sys.exit("FA| did not match:\n" + lines[-1][:400])
g = m.groups()
