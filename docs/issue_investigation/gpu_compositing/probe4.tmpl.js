// Frame-time probe template for the prod kiosk, installed at ~/.surf/script.js.
//
// Times every requestAnimationFrame interval, attributes each frame to app JS
// (wrapped rAF/timer/microtask callbacks) and to forced-synchronous layout (wrapped
// scrollWidth/clientWidth/offsetWidth/getBoundingClientRect/getComputedStyle), then
// exfiltrates a KP| payload through document.title every 2000 ms -- the page's own
// console does not reach the journal on this image. Read it back with xprop.
//
// /*ABLATION*/ marks where a variant's manipulation is spliced in; run-phase.sh
// deploys one variant and collects one capture. parse4.py renders the payload.
(function () {
