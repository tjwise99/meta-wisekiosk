// Ablation D: live scrolling marquee rows capped at 2, reproducing the sparse-data
// condition. Verified in band by the probe's own M<n>/20 count.
(function () {
  ...
  // ABLATION D: cap LIVE scrolling rows at 2, reproducing the earlier sparse-data
  // condition. Remount, geometry reads and property writes are untouched on every row.
  (function () {
    var origAdd = DOMTokenList.prototype.add;
    DOMTokenList.prototype.add = function () {
      var args = Array.prototype.slice.call(arguments);
      if (args.indexOf('marquee') !== -1) {
        if (document.querySelectorAll('.ride-name-text.marquee').length >= 2) return;
      }
