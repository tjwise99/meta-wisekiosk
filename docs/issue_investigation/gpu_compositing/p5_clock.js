// Interleaved clock ablation: does the clock's 1 Hz .seconds repaint set the
// frames>250 ms floor? ON/OFF/ON in one continuous capture, so the board's within-run
// drift is bracketed rather than assumed away. Hidden by style attribute with
// !important, the one injection form the page's CSP permits.
//
// The landing check is in band: getBoundingClientRect().width on .seconds, sampled
// every 2 s per arm. display:none collapses the box to 0 px, so wmax=0 across the OFF
// arm is positive evidence the element was not rendered. parse_arms.py reads it.
(function () {
  ...
  // ABLATION: interleaved ON -> OFF -> ON of the clock's 1 Hz seconds repaint.
  // `.seconds` is the only element the clock rewrites each second (Clock.svelte's `now` is
  // $state, but Svelte skips a set_text whose value is unchanged, so hours/date do not write).
  // Hiding it inline with !important survives the CSP, which drops an injected <style>.
  // Arms by elapsed seconds: 0-20 warmup (excluded), 20-140 ON1, 140-260 OFF, 260-380 ON2.
  var ARM_EDGES = [20, 140, 260, 380];
  var aF = [0, 0, 0], aMs = [0, 0, 0], aBig = [0, 0, 0];
  var aWmax = [-1, -1, -1], aWlast = [-1, -1, -1];
  var selFound = -1, curArm = -2, applied = [0, 0, 0];

  function armOf(elMs) {
    var s = elMs / 1000;
    if (s < ARM_EDGES[0]) return -1;
    if (s < ARM_EDGES[1]) return 0;
    if (s < ARM_EDGES[2]) return 1;
    if (s < ARM_EDGES[3]) return 2;
    return -1;
  }

  function secEl() { return document.querySelector('.seconds'); }

  function applyArm(a) {
    var el = secEl();
    if (!el) { selFound = 0; return; }
    selFound = 1;
    if (a === 1) {
  ...
  function tick(now) {
    ...
    var arm = armOf(el);
    if (arm !== curArm) { curArm = arm; applyArm(arm); }
    if (arm >= 0) {
      aF[arm]++; aMs[arm] += dt;
  ...
  origSetInterval(function () {
    ...
    var sEl = secEl();
    var sW = sEl ? Math.round(sEl.getBoundingClientRect().width) : -1;
    if (curArm >= 0) {
      aWlast[curArm] = sW;
      if (sW > aWmax[curArm]) aWmax[curArm] = sW;
    }

    function arm(i) {
      return 'f' + aF[i] + ':b' + aBig[i] +
             ':m' + (aF[i] ? Math.round(aMs[i] / aF[i]) : 0) +
             ':wmax' + aWmax[i] + ':wlast' + aWlast[i] + ':ap' + applied[i];
    }

    document.title =
      'KA|arm' + curArm + '|sel' + selFound +
      '|ON1_' + arm(0) + '|OFF_' + arm(1) + '|ON2_' + arm(2) + '|' +
